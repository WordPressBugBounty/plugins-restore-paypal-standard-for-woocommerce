...
class rpsfw_Gateway_PayPal_Standard extends WC_Payment_Gateway {
        ...
        switch ( strtolower( $result['ACK'] ) ) {
            ...
            case 'successwithwarning':
                $order->add_order_note(
                    /* translators: %1$s: refund amount, %2$s: PayPal refund transaction ID. */
    ...
    public function admin_scripts() {
        ...
        // Enqueue admin assets. There is no minified build shipped for this
        // script, so load the plain file directly (matches how the Stripe and
        // PayPal Commerce gateways enqueue their admin JS).
        //
        ...
        // until the plugin version is bumped).
        $rpsfw_js_path  = RPSFW_PLUGIN_DIR . 'assets/js/paypal-standard-admin.js';
        $rpsfw_css_path = RPSFW_PLUGIN_DIR . 'assets/css/paypal-standard-admin.css';
        ...
        wp_enqueue_script( 'rpsfw-paypal-admin', RPSFW_PLUGIN_URL . 'assets/js/paypal-standard-admin.js', array( 'jquery' ), $rpsfw_js_ver, true );
        wp_enqueue_style( 'rpsfw-paypal-admin-styles', RPSFW_PLUGIN_URL . 'assets/css/paypal-standard-admin.css', array(), $rpsfw_css_ver );

        // Provide translatable strings to the admin script. Without this the JS
        // falls back to hardcoded English (window.rpsfwPayPalParams).
        wp_localize_script(
            'rpsfw-paypal-admin',
            'rpsfwPayPalParams',
            array(
                'sandboxNotice'       => __( 'SANDBOX MODE ENABLED', 'restore-paypal-standard-for-woocommerce' ),
                'sandboxHelpLinkText' => __( 'Learn how to set up a PayPal Sandbox account', 'restore-paypal-standard-for-woocommerce' ),
                'descriptionText'     => $this->get_option( 'description', __( "Pay with PayPal - You can pay with your credit card if you don't have a PayPal account.", 'restore-paypal-standard-for-woocommerce' ) ),
                // Saved mode, so tabs that don't render the Mode selector
                // (Advanced) can still scope their fields to the active mode.
     ...
     *
     * Returns the configured title (or the default "PayPal") on both admin
     * and frontend so it appears alongside the logo at checkout.
     *
     * @return string
     */
    ...
    public static function log( $message, $level = 'info' ) {
        if ( self::$log_enabled ) {
            ...
            self::$log->log( $level, $message, array( 'source' => 'rpsfw-paypal-standard' ) );
        }
    }
} 